...
int ServerSocket::listenInternal(struct addrinfo *info, const std::string &host) {
    WVLOG(1) << "Will listen on " << host << " " << port_ << " " << info->ai_family;

    int listeningFd = socket(info->ai_family, info->ai_socktype, info->ai_protocol);
    if (listeningFd == -1) {
    ...
    int optval = 1;
    if (setsockopt(listeningFd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) != 0) {
        WPLOG(ERROR) << "Unable to set SO_REUSEADDR option " << host << " " << port_;
    }

    if (info->ai_family == AF_INET6) {
        // for ipv6 address, turn on ipv6 only flag
        if (setsockopt(listeningFd, IPPROTO_IPV6, IPV6_V6ONLY, &optval, sizeof(optval)) != 0) {
...
int ServerSocket::getSelectedPortAndNewAddress(int listeningFd, struct addrinfo &sa, const std::string &host, addrInfoList &infoList) {
    int port;
    ...
    if (res) {
        WLOG(ERROR) << "getaddrinfo failed " << host << " " << port << " : " << gai_strerror(res);
        return -1;
    }
...
ErrorCode ServerSocket::listen() {
    ...
    if (res) {
        ...
        WLOG(ERROR) << "Failed getaddrinfo ai_passive on " << port_ << " : " << res << " : " << gai_strerror(res);
        return CONN_ERROR;
    }
    ...
    for (struct addrinfo *info = infoList; info != nullptr;) {
        ...
        if (port_ == 0) {
            ...
            int selectedPort = getSelectedPortAndNewAddress(listeningFd, sa, host, newInfoList);
            if (selectedPort < 0) {
                ::close(listeningFd);
                info = info->ai_next;
                continue;
        ...
        WVLOG(1) << "Successful listen on " << listeningFd << " host " << host << " port " << port_ << " ai_family " << addressFamily;
...
void ServerSocket::setReceiveBufferSize(int fd) {
    ...
    int status = ::setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &bufSize, sizeof(bufSize));
    if (status != 0) {
        WPLOG(ERROR) << "Failed to set receive buffer " << port_ << " size " << bufSize << " fd " << fd;
        return;
    }
