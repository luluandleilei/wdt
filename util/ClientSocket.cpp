...
ClientSocket::ClientSocket(ThreadCtx &threadCtx, const string &dest, const int port, const EncryptionParams &encryptionParams, int64_t ivChangeInterval)
    : WdtSocket(threadCtx, port, encryptionParams, ivChangeInterval, nullptr), dest_(dest) {
...
ErrorCode ClientSocket::connect() {
    ...
    if (res) {
        ...
        WLOG(ERROR) << "Failed getaddrinfo " << dest_ << " , " << port_ << " : " << res << " : " << gai_strerror(res);
        return CONN_ERROR;
    }
    ...
    for (struct addrinfo *info = infoList; info != nullptr; info = info->ai_next) {
        ...
        if (res < 0) {
            WPLOG(ERROR) << "Failed to make the socket non-blocking " << port_ << " sock " << sockArg << " res " << res;
            closeConnection();
            continue;
        }
        ...
        if (::connect(fd_, info->ai_addr, info->ai_addrlen) != 0) {
            ...
            while (true) {
                ...
                if (threadCtx_.getAbortChecker()->shouldAbort()) {
                    WLOG(ERROR) << "Transfer aborted during connect " << port_ << " " << fd_;
                    closeConnection();
                    return ABORT;
                }
                ...
                int pollTimeout = std::min(connectTimeout - timeElapsed, threadCtx_.getOptions().abort_check_interval_millis);
...
void ClientSocket::setSendBufferSize() {
    ...
    int status = ::setsockopt(fd_, SOL_SOCKET, SO_SNDBUF, &bufSize, sizeof(bufSize));
    if (status != 0) {
        WPLOG(ERROR) << "Failed to set send buffer " << port_ << " size " << bufSize << " fd " << fd_;
        return;
    }
