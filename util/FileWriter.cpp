...
ErrorCode FileWriter::open() {
    ...
    if (fd_ >= 0 && blockDetails_->offset > 0) {
        ...
        if (ret < 0) {
            WPLOG(ERROR) << "Unable to seek to " << blockDetails_->offset << " for " << blockDetails_->fileName;
            close();
        }
    }
...
ErrorCode FileWriter::write(char *buf, int64_t size) {
    ...
    if (!options.skip_writes) {
        ...
        while (count < size) {
            ...
            if (written == -1) {
                if (errno == EINTR) {
                    WVLOG(1) << "Disk write interrupted, retrying " << blockDetails_->fileName;
                    continue;
                }
                WPLOG(ERROR) << "File write failed for " << blockDetails_->fileName
                    << "fd : " << fd_ << " " << written << " " << count << " " << size;
                return FILE_WRITE_ERROR;
            }
            count += written;
        }
        WVLOG(1) << "Successfully written " << count << " bytes to fd " << fd_ << " for file " << blockDetails_->fileName;
        ...
        if (finished && !options.skip_fadvise) {
            ...
            if (posix_fadvise(fd_, blockDetails_->offset, blockDetails_->dataSize, POSIX_FADV_DONTNEED) != 0) {
                WPLOG(ERROR) << "posix_fadvise failed for " << blockDetails_->fileName
                    << " " << blockDetails_->offset << " " << blockDetails_->dataSize;
            }
        }
#endif
    }
...
void FileWriter::syncFileRange(int64_t written, bool forced) {
    ...
    if (forced || writtenSinceLastSync_ > syncIntervalBytes) {
        ...
        int status;
        {
            PerfStatCollector statCollector(threadCtx_, PerfStatReport::SYNC_FILE_RANGE);
            status = sync_file_range(fd_, nextSyncOffset_, writtenSinceLastSync_, SYNC_FILE_RANGE_WRITE);
        }
        if (status != 0) {
        ...
        WVLOG(1) << "file range [" << nextSyncOffset_ << " " << writtenSinceLastSync_ 
