...
int FileCreator::openAndSetSize(ThreadCtx &threadCtx, BlockDetails const *blockDetails) {
    int fd;
...
int FileCreator::openExistingFile(ThreadCtx &threadCtx, const string &relPathStr) {
...
int FileCreator::createFile(ThreadCtx &threadCtx, const string &relPathStr) {
    ...
    if (p) {
        ...
        bool dirSuccess1;
        {
            PerfStatCollector statCollector(threadCtx, PerfStatReport::DIRECTORY_CREATE);
            dirSuccess1 = createDirRecursively(dir);
        }
        if (!dirSuccess1) {
            // retry with force
            WLOG(ERROR) << "failed to create dir " << dir << " recursively, " << "trying to force directory creation";
            bool dirSuccess2;
            {
                PerfStatCollector statCollector(threadCtx, PerfStatReport::DIRECTORY_CREATE);
    ...
    if (threadCtx.getOptions().overwrite || threadCtx.getOptions().enable_download_resumption) {
    ...
    if (res < 0) {
        ...
        WPLOG(ERROR) << "failed creating file " << path << ", trying to " << "force directory creation";
        bool dirSuccess;
        {
            PerfStatCollector statCollector(threadCtx, PerfStatReport::DIRECTORY_CREATE);
