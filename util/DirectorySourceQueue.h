...
public:
     ...
     */
    DirectorySourceQueue(const WdtOptions &options, const std::string &rootDir, IAbortChecker const *abortChecker);
...
private:
    ...
    struct SourceComparator {
        bool operator()(const std::unique_ptr<ByteSource> &source1, const std::unique_ptr<ByteSource> &source2) {
            bool toBeDeleted1 = (source1->getMetaData().allocationStatus == TO_BE_DELETED);
            bool toBeDeleted2 = (source2->getMetaData().allocationStatus == TO_BE_DELETED);
     ...
     * time.
     */
    std::priority_queue<std::unique_ptr<ByteSource>, std::vector<std::unique_ptr<ByteSource>>, SourceComparator> sourceQueue_;
