...
public:
    ...
    int64_t encodeLogHeader(char *dest, int64_t max, const std::string &recoveryId, const std::string &senderIp, int64_t config);

    /// decodes header entry
    bool decodeLogHeader(char *buf, int16_t size, int64_t &timestamp, int &version, std::string &recoveryId, std::string &senderIp, int64_t &config);

    /// encodes file creation entry
    int64_t encodeFileCreationEntry(char *dest, int64_t max, const std::string &fileName, const int64_t seqId, const int64_t fileSize);

    /// decodes file creation entry
    bool decodeFileCreationEntry(char *buf, int16_t size, int64_t &timestamp, std::string &fileName, int64_t &seqId, int64_t &fileSize);

    /// encodes block write entry
    int64_t encodeBlockWriteEntry(char *dest, int64_t max, const int64_t seqId, const int64_t offset, const int64_t blockSize); 

    /// decodes block write entry
    bool decodeBlockWriteEntry(char *buf, int16_t size, int64_t &timestamp, int64_t &seqId, int64_t &offset, int64_t &blockSize);

    /// encodes file resize entry
    int64_t encodeFileResizeEntry(char *dest, int64_t max, const int64_t seqId, const int64_t fileSize);

    /// decodes file resize entry
    bool decodeFileResizeEntry(char *buf, int16_t size, int64_t &timestamp, int64_t &seqId, int64_t &fileSize);

    /// encodes file invalidation entry
    int64_t encodeFileInvalidationEntry(char *dest, int64_t max, const int64_t &seqId);

    /// decodes file invalidation entry
    bool decodeFileInvalidationEntry(char *buf, int16_t size, int64_t &timestamp, int64_t &seqId);
    ...
    bool decodeDirectoryInvalidationEntry(char *buf, int16_t size, int64_t &timestamp);
...
public:
     ...
     */
    void addFileCreationEntry(const std::string &fileName, int64_t seqId, int64_t fileSize);
...
private:
     ...
     */
    ErrorCode parseVerifyAndFix(const std::string &recoveryId, int64_t config, bool parseOnly, std::vector<FileChunksInfo> &parsedInfo);
