...
DirectorySourceQueue::DirectorySourceQueue(const WdtOptions &options, const string &rootDir, IAbortChecker const *abortChecker) {
    threadCtx_ = std::make_unique<ThreadCtx>(options, /* do not allocate buffer */ false);
...
void DirectorySourceQueue::setFileInfo(const std::vector<WdtFileInfo> &fileInfo) {
    fileInfo_ = fileInfo;
...
std::vector<SourceMetaData *> &DirectorySourceQueue::getDiscoveredFilesMetaData() {
...
void DirectorySourceQueue::setPreviouslyReceivedChunks(std::vector<FileChunksInfo> &previouslyTransferredChunks) {
    ...
    for (auto &chunkInfo : previouslyTransferredChunks) {
        nextSeqId_ = std::max(nextSeqId_, chunkInfo.getSeqId() + 1); //设置nextSeqId_为重传前的SeqId的最大值加1(新文件的SeqId不会与重传前的文件SeqId相同，以唯一标识文件)
        auto fileName = chunkInfo.getFileName();
        previouslyTransferredChunks_.insert( std::make_pair(std::move(fileName), std::move(chunkInfo)));
    }
...
bool DirectorySourceQueue::buildQueueSynchronously() {
    ...
    if (exploreDirectory_) {
        ...
        WLOG(INFO) << "Using list of file info. Number of files " << fileInfo_.size();
        res = enqueueFiles();
    }
    ...
    WVLOG(1) << "finished initialization of DirectorySourceQueue in " << directoryTime_;
    return res;
}
...
bool DirectorySourceQueue::explore() {
    WLOG(INFO) << "Exploring root dir " << rootDir_ << " include_pattern : " << includePattern_ << " exclude_pattern : " << excludePattern_ << " prune_dir_pattern : " << pruneDirPattern_;
    ...
    while (!todoList.empty()) {
        ...
        while (true) {
            ...
            WVLOG(2) << "Found entry " << dirEntryRes->d_name << " type " << (int)dType;
            ...
            if (!isDir) {
                ...
                if (followSymlinks_ && isLink) {
                    ...
                    WVLOG(2) << "Resolved symlink " << dirEntryRes->d_name << " to " << newFullPath;
                }
                ...
                if (S_ISREG(fileStat.st_mode)) {
                    WVLOG(2) << "Found file " << newFullPath << " of size " << fileStat.st_size;

                    if (!excludePattern_.empty() && std::regex_match(newRelativePath, excludeRegex)) {
                        continue;
                    }

                    if (!includePattern_.empty() && !std::regex_match(newRelativePath, includeRegex)) {
                        continue;
                    }
            ...
            if (isDir) {
                if (followSymlinks_) {
                    if (visited.find(newFullPath) != visited.end()) {
                        WLOG(ERROR) << "Attempted to visit directory twice: " << newFullPath;
                        hasError = true;
                        continue;
                    }
                ...
                if (pruneDirPattern_.empty() || !std::regex_match(newRelativePath, pruneDirRegex)) {
    ...
    WLOG(INFO) << "Number of files explored: " << numEntries_ << " opened " << numFilesOpened_ << " with direct " << numFilesOpenedWithDirect_ << " errors " << std::boolalpha << hasError;
    return !hasError;
}
...
void DirectorySourceQueue::returnToQueue(std::vector<std::unique_ptr<ByteSource>> &sources) {
    int returnedCount = 0;
    std::unique_lock<std::mutex> lock(mutex_);
...
void DirectorySourceQueue::createIntoQueue(const string &fullPath, WdtFileInfo &fileInfo) {
    ...
    if ((openFilesDuringDiscovery_ != 0) && (metadata->fd < 0)) {
        metadata->fd = FileUtil::openForRead(*threadCtx_, fullPath, metadata->directReads);
        ++numFilesOpened_;
        ...
        if (--openFilesDuringDiscovery_ == 0) {
            WLOG(WARNING) << "Already opened " << numFilesOpened_ << " files, will open the reminder as they are sent";
        }
    }
...
void DirectorySourceQueue::createIntoQueueInternal(SourceMetaData *metadata) {
    ...
    if (it == previouslyTransferredChunks_.end()) {
        ...
        seqId = nextSeqId_++;
        allocationStatus = NOT_EXISTS;  //receiver 不存在该文件
    } else if (it->second.getFileSize() > fileSize) {
        // file size is greater on the receiver side
        remainingChunks.emplace_back(0, fileSize);
        seqId = nextSeqId_++;
        WLOG(INFO) << "File size is greater in the receiver side " << relPath << " " << fileSize << " " << it->second.getFileSize();
        ...
        allocationStatus = it->second.getFileSize() < fileSize ? EXISTS_TOO_SMALL : EXISTS_CORRECT_SIZE;
    }
    ...
    //将所有的chunks划分为block，存储到sourceQueue中
    for (const auto &chunk : remainingChunks) {
        ...
        do {
            ...
            std::unique_ptr<ByteSource> source = std::make_unique<FileByteSource>(metadata, size, offset);
            sourceQueue_.push(std::move(source));
            remainingBytes -= size;
            offset += size;
            blockCount++;
...
std::vector<TransferStats> &DirectorySourceQueue::getFailedSourceStats() {
    while (!sourceQueue_.empty()) {
        failedSourceStats_.emplace_back(std::move(sourceQueue_.top()->getTransferStats()));
        sourceQueue_.pop();
    }
...
bool DirectorySourceQueue::enqueueFiles() {
    for (auto &info : fileInfo_) {
        if (threadCtx_->getAbortChecker()->shouldAbort()) { //abort检查
            WLOG(ERROR) << "Directory transfer thread aborted";
            return false;
        }

        string fullPath = rootDir_ + info.fileName; //获取文件的绝对路径

        if (info.fileSize < 0) { //获取文件大小
            struct stat fileStat;
...
void DirectorySourceQueue::enqueueFilesToBeDeleted() {
    ...
    for (auto &it : previouslyTransferredChunks_) {
        ...
        WLOG(INFO) << "Extra file " << fileName << " seq-id " << seqId << " on the receiver side, will be deleted";
        SourceMetaData *metadata = new SourceMetaData();
        ...
        std::unique_ptr<ByteSource> source = std::make_unique<FileByteSource>(metadata, 0, 0);
        sourceQueue_.push(std::move(source));
        numFilesToBeDeleted++;
    }
...
std::unique_ptr<ByteSource> DirectorySourceQueue::getNextSource(ThreadCtx *callerThreadCtx, ErrorCode &status) {
    ...
    while (true) {
        ...
        source = std::move( const_cast<std::unique_ptr<ByteSource> &>(sourceQueue_.top()));
        sourceQueue_.pop();
        ...
        WVLOG(1) << "got next source " << rootDir_ + source->getIdentifier() << " size " << source->getSize();
