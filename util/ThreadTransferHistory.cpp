...
ErrorCode ThreadTransferHistory::setGlobalCheckpoint(const Checkpoint &checkpoint) {
    ...
    while (inUse_) {
        ...
        WLOG(INFO) << "Transfer history still in use, waiting, checkpoint " << checkpoint;
...
ErrorCode ThreadTransferHistory::setCheckpointAndReturnToQueue( const Checkpoint &checkpoint, bool globalCheckpoint) {
    ...
    if (numReceivedSources > historySize) { 
        WLOG(ERROR) << "checkpoint is greater than total number of sources transferred " << history_.size() << " " << numReceivedSources;
    ...
    for (int64_t i = 0; i < numFailedSources; i++) {
        ...
        history_.pop_back();
        const Checkpoint *checkpointPtr = (i == numFailedSources - 1 ? &checkpoint : nullptr);
        markSourceAsFailed(source, checkpointPtr);
        sourcesToReturn.emplace_back(std::move(source));
    }

    queue_.returnToQueue(sourcesToReturn);
    WLOG(INFO) << numFailedSources << " number of sources returned to queue, checkpoint: " << checkpoint;
    return errCode;
}
...
ErrorCode ThreadTransferHistory::validateCheckpoint(const Checkpoint &checkpoint, bool globalCheckpoint) {
    ...
    if (checkpoint.numBlocks < lastCheckpoint_->numBlocks) {
        WLOG(ERROR) << "Current checkpoint must be higher than previous checkpoint, Last checkpoint: "
    ...
    if (checkpoint.lastBlockSeqId == lastCheckpoint_->lastBlockSeqId &&
        ...
        // same block
        if (checkpoint.lastBlockReceivedBytes != lastCheckpoint_->lastBlockReceivedBytes) {
    ...
    if (noProgress && !globalCheckpoint) {
        // we can get same global checkpoint multiple times, so no need to check for progress
...
void ThreadTransferHistory::markSourceAsFailed( std::unique_ptr<ByteSource> &source, const Checkpoint *checkpoint) {
    auto &metadata = source->getMetaData();
    bool validCheckpoint = false;
    if (checkpoint != nullptr) {
        if (checkpoint->hasSeqId) {
            if ((checkpoint->lastBlockSeqId == metadata.seqId) && (checkpoint->lastBlockOffset == source->getOffset())) {
    ...
    int64_t receivedBytes = (validCheckpoint ? checkpoint->lastBlockReceivedBytes : 0);
...
void TransferHistoryController::handleGlobalCheckpoint( const Checkpoint &checkpoint) {
    auto errPort = checkpoint.port;
    auto it = threadHistoriesMap_.find(errPort);
    if (it == threadHistoriesMap_.end()) {
        WLOG(ERROR) << "Invalid checkpoint " << checkpoint << ". No sender thread running on port " << errPort;
        return;
    }
