...
void WdtBase::setProgressReporter( std::unique_ptr<ProgressReporter>& progressReporter) {
...
void WdtBase::checkAndUpdateBufferSize() {
    ...
    if (bufSize < Protocol::kMaxHeader) {
        ...
        WLOG(WARNING) << "Specified buffer size " << options_.buffer_size << " less than " << Protocol::kMaxHeader << ", using " << bufSize;
    }
    if (bufSize % kDiskBlockSize != 0) {
        int64_t alignedBufSize = ((bufSize + kDiskBlockSize - 1) / kDiskBlockSize) * kDiskBlockSize;
        WLOG(WARNING) << "Buffer size " << bufSize << " not divisible by disk block size " << kDiskBlockSize << ", changing it to " << alignedBufSize;
...
ErrorCode WdtBase::validateTransferRequest() {
    ...
    if (code != OK) {
        WLOG(ERROR) << "WDT object initiated with erroneous transfer request " << transferRequest_.getLogSafeString();
        return code;
    }

    if (transferRequest_.directory.empty() || (transferRequest_.protocolVersion < 0) || transferRequest_.ports.empty()) {
        WLOG(ERROR) << "Transfer request validation failed for wdt object " << transferRequest_.getLogSafeString();
